...
namespace BD_Comm_MySQL
{
    ...
    {   //prototype:
        ...
        //Métodos:
        public BDMSSQL(string cadena_de_conexion)  //Constructor
        {
        ...
        public DataTable LeerTabla(string _tabla)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(_cadconn))
            {
                cnn.Open();
                SqlDataAdapter da = new SqlDataAdapter(iSql, cnn);
         ...
         *********************************************************************/
        {
            ...
            if (_datos[0] is string)
                ...
                iSql = iSql + fechayhoraMSSQL((DateTime)_datos[0]);
            else
                iSql = iSql + _datos[0];
            ...
            for (int x = 1; x < _datos.Length; x++)
            {
                if (_datos[x] is string)
                    ...
                    iSql = iSql + ", '" + fechayhoraMSSQL((DateTime)_datos[x]) + "'";
                else
                    iSql = iSql + ", " + _datos[x];
        ...
        /// <summary>
        /// Devuelve una fecha en formato compatible con MSSQL Server.
        /// </summary>
        /// <param name="_fecha">Un DateTime conteniendo la fecha.</param>
        /// <returns>Retorna un string conteniendo la fecha en formato MSSQL Server.</returns>
        private string fechaMSSQL(DateTime _fecha)
        {
        ...
        /// <summary>
        /// Devuelve un DateTime en formato compatible con MSSQL Server.
        /// </summary>
        /// <param name="_tiempo">Un DateTime conteniendo la fecha y hora.</param>
        /// <returns>Retorna un string conteniendo la fecha y la hora en formato MSSQL Server.</returns>
        private string fechayhoraMSSQL(DateTime _tiempo)
        {
         ...
         *********************************************************************/
        {
            ...
            if (_datos[0] is string)
                ...
                iSql = iSql + _campos[0] + " = '" + fechayhoraMSSQL((DateTime)_datos[0]) + "'";
            else
            ...
            for (int x = 1; x < _datos.Length; x++)
            {
                if (_datos[x] is string)
                    ...
                    iSql = iSql + ", " + _campos[x] + " = '" + fechayhoraMSSQL((DateTime)_datos[x]) + "'";
                else
            ...
            if (_valorkey is string)
                ...
                iSql = iSql + " WHERE " + _key + "='" + fechayhoraMSSQL((DateTime)_valorkey) + "';";
            else
